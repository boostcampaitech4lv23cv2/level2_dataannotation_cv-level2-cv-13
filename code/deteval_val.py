...
def calc_deteval_metrics(pred_bboxes_dict, gt_bboxes_dict, transcriptions_dict=None,
    ...
    numGt = 0
    numDet = 0
    first_key = next(iter(gt_bboxes_dict))
    result_matrix=dict()
    ...
    for sample_name in gt_bboxes_dict:
        ...
        if sample_name in pred_bboxes_dict:
            ...
            if len(detRects)>0:
                ...
                output_match = np.zeros(outputShape)
                ...
                for gtNum in range(len(gtRects)):
                    for detNum in range(len(detRects)):
                        ...
                        if recallMat[gtNum,detNum] >= eval_hparams['AREA_RECALL_CONSTRAINT'] and precisionMat[gtNum,detNum] >= eval_hparams['AREA_PRECISION_CONSTRAINT']:
                            output[gtNum,detNum] = 1
                        else:
                            output[gtNum,detNum] = 0
                        # output[gtNum,detNum] = 1 if recallMat[gtNum,detNum] >= eval_hparams['AREA_RECALL_CONSTRAINT'] and precisionMat[gtNum,detNum] >= eval_hparams['AREA_PRECISION_CONSTRAINT'] else 0
                        ...
                        if match is True :
                            ...
                            if num_overlaps_gt(gtNum)>=2 :
                                ...
                                for detNum in matchesDet :
                                    detRectMat[detNum] = 1
                                    output_match[gtNum,detNum] = eval_hparams['MTYPE_OO_O'] if len(matchesDet)==1 else eval_hparams['MTYPE_OM_O']
                        ...
                        if match is True :
                            ...
                            if num_overlaps_det(detNum)>=2 :
                                ...
                                for gtNum in matchesGt :
                                    gtRectMat[gtNum] = 1
                                    output_match[gtNum,detNum] = eval_hparams['MTYPE_OO_O'] if len(matchesGt)==1 else eval_hparams['MTYPE_OM_M']
        ...
        real = output * output_match
        result_matrix[sample_name]=[sample_name, real,output,output_match, precisionMat, recallMat, precision, recall]
